...
{
    public abstract class AntTreeViewBase<TModel> : TreeViewBase<TModel>
    {
        ...
        public List<TModel> GetChildren(TModel data)
        {
            ...
            return DataListTItem.Where(item => ((int?)item.GetType().GetProperty("ParentId").GetValue(item)) == id).ToList();
